...
const SearchBar = ({restaurants, setFilteredRestaurants}) => {
    ...
    return(
        <form className="search-container flex gap-4 pt-6 w-[100%] justify-center">
            ...
            <input type="search" placeholder="Search for restaurant..." value={searchInput} className="search-bar outline-0 border border-solid border-gray-400 w-[60%] h-10 rounded-md py-4 px-10 text-base placeholder:text-base focus:border-myBlack bg-myGray transition-all duration-300" name="search-bar" onChange={(e) => {

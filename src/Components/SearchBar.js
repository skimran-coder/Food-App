...
import useLanding from "../utils/useLanding"
...
const SearchBar = () => {
    ...
    return(
        ...
        <form className="search-container flex gap-4 pt-6 w-[100%] justify-center relative">
            ...
            <input type="search" placeholder="Search for restaurants and food" value={searchInput} className="search-bar outline-0 border border-solid border-gray-400 w-full h-10 rounded-md py-4 px-10 text-base placeholder:text-base focus:border-myYellow  transition-all duration-300" name="search-bar" onChange={(e) => {
                setSearchInput(e.target.value);

            }} />
            {
             !searchInput ? 
            <i className="fa fa-search absolute opacity-50 right-8 top-9" aria-hidden="true"></i>
            :
            <i className="fa-solid fa-x absolute opacity-50 right-8 top-9 cursor-pointer" onClick={() => setSearchInput("")}></i>
            }

...
import { Empty_IMG, IMG_CDN_URL, nonVegIcon, vegIcon } from "../Constant";
...
const Cart = () => {
    ...
    const deliveryFees = Math.floor(price * 0.10)
    const platformFee = 5
    const gst = Math.floor((price + deliveryFees + platformFee) * 0.10)

    useEffect(() => {
        const priceData = cartItems?.map((items) => items?.card?.info?.price || items?.card?.info?.defaultPrice)
            ...
            {cartItems.length !== 0 && 
                dataToDisplay.map((item) => (
                    <div className="flex justify-center items-center shadow-lg gap-4 px-5 py-4">
                        ...
                        <img src={IMG_CDN_URL + item?.card?.info?.imageId} className="w-12 h-12 aspect-square object-cover rounded-lg  "/>
                        <h1 className="  w-60">{item?.card?.info?.name}</h1>
                        <div className=" w-fit"><Buttons item={item}/></div>
                        <h3 className=" leading-6  w-fit ">{ item?.card?.info?.price /100 ? "₹" + item?.card?.info?.price /100 : "₹" + item?.card?.info?.defaultPrice /100}</h3>
